...
class Apply extends MY_Controller {
	...
	public function __construct(){
		...
		$this->load->database();     
        $this->load->model('juggler/Applymodel');
		date_default_timezone_set('Asia/Kolkata');
    }
 	public function index()
    {    	
        $PageQry = $this->Applymodel->getApply();
        $this->load->view('juggler/apply', ['PageQry'=>$PageQry]);
    }

    public function add(){
        $this->load->view('juggler/add_apply');
    }

	public function insert(){
	
		$config['upload_path']   = './assets/images/apply/';
		...
		$res = $this->db->insert('tbl_how_to_apply', $data);
		if($res==true){		
		$this->session->set_flashdata('success', '<div class="alert alert-success fade show"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong><i class="fa fa-check-circle" aria-hidden="true"></i>&nbsp;Sucess!&nbsp;&nbsp;</strong>Row Added.</div>');
		    ...
		    redirect('juggler/apply');
	}

	public function edit($id){
		$PageQry = $this->Applymodel->getApplyByID($id);
		$this->load->view('juggler/edit_apply', ['PageQry'=>$PageQry]);
	}
	...
	public function update(){
	  ...
	  $image = $this->upload->data();
		...
		$this->db->where('id', $id);
		$res = $this->db->update('tbl_how_to_apply', $data);
		if($res==true){		
	...
	public function delete($id){
		$res = $this->Applymodel->deleteApplyByID($id);
		if($res==true){		
