...
class Testimonials extends MY_Controller {
	...
	public function __construct(){
		...
		$this->load->database();     
        $this->load->model('juggler/Testimonialsmodel');
		date_default_timezone_set('Asia/Kolkata');
    }
 	public function index()
    {    	
        $PageQry = $this->Testimonialsmodel->getTestimonials();
        $this->load->view('juggler/testimonials', ['PageQry'=>$PageQry]);
    }

    public function add(){
        $this->load->view('juggler/add_testimonials');
    }
	...
	public function insert(){
		...
		$this->session->set_flashdata('success', '<div class="alert alert-success fade show"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong><i class="fa fa-check-circle" aria-hidden="true"></i>&nbsp;Sucess!&nbsp;&nbsp;</strong>Row Added.</div>');
		    ...
		    redirect('juggler/testimonials');
	}

	public function edit($id){
		$PageQry = $this->Testimonialsmodel->getTestimonialsByID($id);
		$this->load->view('juggler/edit_testimonials', ['PageQry'=>$PageQry]);
	}
	...
	public function update(){
	  ...
	  $image = $this->upload->data();
		...
		$this->db->where('id', $id);
		$res = $this->db->update('tbl_testimonials', $data);
		if($res==true){		
	...
	public function delete($id){
		$res = $this->Testimonialsmodel->deleteTestimonialsByID($id);
		if($res==true){		
